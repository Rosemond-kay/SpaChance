...
export default function GiftCardModal({ isOpen, onClose }) {
  ...
  const handleOrder = (channel) => {
    ...
    setTimeout(() => {
      if (channel === 'whatsapp') {
        window.open(`${BRAND.whatsappUrl}?text=${encodeURIComponent(cardSummary)}`, '_blank');
      } else {
      ...
      >
        ...
        <div
          style={{
            ...
            marginBottom: '1.8rem',
            position: 'relative',
            backgroundImage: 'radial-gradient(circle at 90% 10%, rgba(140, 128, 112, 0.25) 0%, transparent 60%)',
            boxShadow: '0 15px 35px rgba(0,0,0,0.2)'
          }}
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem' }}>
            <div>
              <span style={{ fontFamily: 'var(--font-serif)', fontSize: '1.5rem', fontWeight: 600, color: '#e4d3b7' }}>
                SpaChance
              </span>
              <span style={{ display: 'block', fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '0.15em', color: '#ab9f8f' }}>
          ...
          <div style={{ marginBottom: '1rem' }}>
            <span style={{ fontSize: '0.75rem', textTransform: 'uppercase', color: '#ab9f8f', letterSpacing: '0.08em' }}>Gift Voucher Value</span>
            <div style={{ fontFamily: 'var(--font-serif)', fontSize: '2.2rem', color: '#e4d3b7', fontWeight: 'bold' }}>
              GH₵ {amount}
            </div>
          </div>

          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.82rem', color: '#ab9f8f', borderTop: '1px solid rgba(228, 211, 183, 0.2)', paddingTop: '0.8rem' }}>
            <div>To: <span style={{ color: '#e4d3b7' }}>{recipient || 'Honored Guest'}</span></div>
            <div>From: <span style={{ color: '#e4d3b7' }}>{sender || 'A Special Friend'}</span></div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
              ...
              {presetAmounts.map((val) => (
                <button
                  ...
                  style={{
                    padding: '0.6rem',
                    borderRadius: '4px',
                    border: amount === val ? '2px solid var(--text-primary)' : '1px solid var(--border-subtle)',
                    backgroundColor: amount === val ? 'var(--text-primary)' : 'rgba(255,255,255,0.5)',
                    color: amount === val ? 'var(--bg-primary)' : 'var(--text-primary)',
                    fontWeight: 600,
                    cursor: 'pointer'
                  }}
                >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            <div>
              <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 600, marginBottom: '0.3rem' }}>Recipient Name:</label>
              <input
                type="text"
          ...
          <div>
            <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 600, marginBottom: '0.3rem' }}>Personal Message (Optional):</label>
            <textarea
              rows={2}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginTop: '0.5rem' }}>
            ...
            >
              ...
              {ordered ? 'Generating Gift Order...' : 'Order Gift Card via WhatsApp'}
            </button>
