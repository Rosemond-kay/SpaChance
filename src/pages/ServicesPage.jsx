...
export default function ServicesPage({ onBookService, onOpenGiftModal }) {
  ...
  const currentCatalog = SERVICES_CATALOG.find((c) => c.category === activeCategory) || SERVICES_CATALOG[0];
  ...
  return (
    <div className="services-page">
      ...
      >
        ...
        <div className="container" style={{ position: 'relative', zIndex: 2 }}>
          <div style={{ maxWidth: '680px', margin: '0 auto' }}>
            ...
            >
              Explore our curated spa packages, skin barrier facials, therapeutic deep tissue massage, and beauty treatments in East Legon, Accra.
            </p>
      ...
      <section style={{ backgroundColor: '#e5d7be', borderBottom: '1px solid var(--border-subtle)' }}>
        <div className="container" style={{ display: 'flex', justifyContent: 'center', gap: '0.6rem', padding: '0.85rem', overflowX: 'auto' }}>
          {categories.map((cat) => (
            <button
              key={cat}
              ...
              style={{
                ...
                backgroundColor: activeCategory === cat ? 'var(--text-primary)' : 'rgba(255, 255, 255, 0.5)',
            ...
            >
              {cat === 'Packages' && <Sparkles size={13} style={{ color: activeCategory === 'Packages' ? '#ede0c8' : 'var(--accent-dark)' }} />}
              {cat}
            </button>
          ))}
        </div>
      ...
      <section className="section-padding" style={{ backgroundColor: 'var(--bg-primary)' }}>
        <div className="container">
          ...
          <div
            style={{
              display: 'grid',
              gridTemplateColumns: activeCategory === 'Packages' ? 'repeat(auto-fit, minmax(310px, 1fr))' : 'repeat(auto-fit, minmax(280px, 1fr))',
              gap: '1.8rem'
            }}
          >
            {currentCatalog.items.map((item) => (
              <div
                ...
                style={{
                  ...
                  border: item.popular ? '2px solid var(--accent-dark)' : '1px solid var(--border-subtle)'
                }}
              >
                <div>
                  {item.badge && (
                    <span className="featured-badge" style={{ marginBottom: '0.6rem' }}>
                      {item.badge}
                    </span>
                  )}
                  <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: '1.35rem', marginBottom: '0.5rem' }}>{item.title}</h3>
                  ...
                  {item.includes && (
                    <div style={{ backgroundColor: 'rgba(255, 255, 255, 0.4)', padding: '0.85rem', borderRadius: '4px', marginBottom: '1rem' }}>
                      <span style={{ fontSize: '0.72rem', fontWeight: 600, textTransform: 'uppercase', color: 'var(--accent-dark)', display: 'block', marginBottom: '0.3rem' }}>Included:</span>
                      <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
                        {item.includes.map((inc, i) => (
                          <li key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.82rem' }}>
                            <Check size={13} style={{ color: 'var(--accent-dark)' }} /> {inc}
                          </li>
                        ))}
                      </ul>
                    </div>
                ...
                <div style={{ paddingTop: '1rem', borderTop: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div>
                    <span style={{ fontSize: '0.78rem', color: 'var(--text-light)', display: 'block' }}>{item.duration}</span>
      ...
      <section className="section-padding" style={{ backgroundColor: '#e5d7be' }}>
        <div className="container" style={{ textAlign: 'center', maxWidth: '680px' }}>
          <Gift size={28} style={{ color: 'var(--accent-dark)', marginBottom: '0.6rem' }} />
